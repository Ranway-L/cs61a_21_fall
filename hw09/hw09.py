...
def roman_numerals(text):
    ...
    []
    """
    return re.findall(r'\b[IVXLCDM]+\b', text)            #用\b锁定某个单词
...
def cs_classes(post):
    ...
    True
    """
    return bool(re.search(r'\b(CS|cs) ?\d+[abcABC]?\b', post))
...
def match_time(text):
    ...
    ['2:00']
    """
    return re.findall(r'(?:(?:[01]?\d)|(?:[2][0-3])):[0-5]\d(?:AM|PM)?', text)            #？？？？？
    # return re.findall(r'(([01]?\d)|([2][0-3])):[0-5]\d(?:AM|PM)?', text)                #这个为什么是错误的？
    #见下题，()在findall中使用时，会返回()内的内容
import re
...
def area_codes(text):
    ...
    []
    """
    return re.findall(r'\b\(?(\d{3})\)? ?\d{3}? ?\d{4}\b', text)  #当使用()时，findall会返回()找到的内容
...
def most_common_code(text):
    ...
    """
    "*** YOUR CODE HERE ***"
    all_head = area_codes(text)
    return max(all_head, key = lambda x: all_head.count(x))