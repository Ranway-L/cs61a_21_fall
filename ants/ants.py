...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            # BEGIN Problem 8
            print('DEBUG: place.ant.is_container ',place.ant.is_container )
            print('DEBUG: place.ant.can_contain(self) ',place.ant.can_contain(self))
            print('DEBUG: self.is_container ',self.is_container)
            print('DEBUG: self.can_contain(place.ant) ',self.can_contain(place.ant))
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while any([not now_place.bees, self.min_range > my_range ,self.max_range < my_range ]):
            my_range += 1
            now_place = now_place.entrance
            if now_place.is_hive == True: #or now_place == None:
                return None
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8
        "*** YOUR CODE HERE ***"
        print('DEBUG: In can_contain function: self.ant_contained ',self.ant_contained)
        if self.ant_contained == None:
            print('DEBUG: self.ant_contained != None')
            if not isinstance(other,ContainerAnt):
                print('DEBUG: can_contain function operates correctly')
                return True
        else:
            return False
        # END Problem 8
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        # BEGIN Problem 12
        "*** YOUR CODE HERE ***"
        if gamestate.Queen_exist:
            return None
        gamestate.Queen_exist = True
        return super().construct(gamestate)        #漏掉了前面的return，返回值默认成为None
        # END Problem 12
    ...
    def action(self, gamestate):
        ...
        while myplace:
            if myplace.ant:
                print('DEBUG: Buff them!')
                if myplace.ant.is_container and myplace.ant.ant_contained:
                    myplace.ant.ant_contained.buff()
                myplace.ant.buff()
...
class GameState:
    ...
    def __init__(self, strategy, beehive, ant_types, create_places, dimensions, Queen_exist = False, food=2):
