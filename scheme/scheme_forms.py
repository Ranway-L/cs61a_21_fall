...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 4
        "*** YOUR CODE HERE ***"
        env.define(signature, scheme_eval(expressions.rest.first, env))      #Pair.rest 会自动在后面的内容加()，用first来消除这个括号，而不是用first来取rest的第一个数据
        return signature
        # END PROBLEM 4
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        func_formals = signature.rest
        func_body = expressions.rest
        # env.define(signature.first, LambdaProcedure(formals, body, env))     #这种方法比下面的简单
        env.define(signature.first, do_lambda_form(Pair(func_formals, func_body), env))        #按照Hint做的方法，但是这种方法反而麻烦
        # print('DEBUG: env: ', env.bindings)
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    while expressions != nil:
        value = scheme_eval(expressions.first, env)    #用value保存eval的值，而不是在if判断和return中执行两次。若执行两次可能导致累加结果的多加
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if clause.first == 'else':
            ...
            # print('DEBUG: clause.rest', clause.rest)
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        values = Pair(eval_all(a.rest, env), values)     #新建Pair来实现链表的扩增
